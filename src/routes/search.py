...
async def search_tag(tag: str = Path(min_length=2), rating_low: float = Query(0),
    ...
    try:
        ...
        photos = await search_by_tag(tag, rating_low, rating_high, start_date, end_date, db)
...
async def search_description(description: str = Path(min_length=2), rating_low: float = Query(0),
                             rating_high: float = Query(999),
                             start_data: str = Query(datetime.now().date() - timedelta(days=365 * 60)),
                             end_data: str = Query(datetime.now().date()),
                             current_user: User = Depends(auth_service.get_authenticated_user),
    ...
    try:
        start_date = datetime.strptime(start_data, '%Y-%m-%d').date()
        end_date = datetime.strptime(end_data, '%Y-%m-%d').date()
        photos = await search_by_description(description, rating_low, rating_high, start_date, end_date, db)
        return {"photos": photos}
    except ValueError as e:
        return {"details": BAD_DATE_FORMAT}


@router.get("/admin_search/{user_id}", dependencies=[Depends(Admin_Moder)], tags=["Search"])
async def admin_search(user_id: int, text: str = Query(None, min_length=2), rating_low: float = Query(0),
                       rating_high: float = Query(999),
    ...
    try:
        start_date = datetime.strptime(start_data, '%Y-%m-%d').date()
        end_date = datetime.strptime(end_data, '%Y-%m-%d').date()
        photos = await search_admin(user_id, text, rating_low, rating_high, start_date, end_date, db)
        return {"photos": photos}
    except ValueError as e:
        return {"details": BAD_DATE_FORMAT}
