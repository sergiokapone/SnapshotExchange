...
@router.get("/{description}", tags=["Search"])
async def search_photo(description: str = Path(min_length=2), rating_low: float = Query(0),
                       rating_high: float = Query(999),
                       start_data: str = Query(str(datetime.now().date() - timedelta(days=365 * 60))),
                       end_data: str = Query(str(datetime.now().date())),
                       current_user: User = Depends(auth_service.get_authenticated_user),
...
async def admin_search(user_id: int, tag: str = Query(None, min_length=2), rating_low: float = Query(0),
                       rating_high: float = Query(999),
                       start_data: str = Query(str(datetime.now().date() - timedelta(days=365 * 60))),
                       end_data: str = Query(str(datetime.now().date())),
                       current_user: User = Depends(auth_service.get_authenticated_user),
    ...
    try:
        start_date = datetime.strptime(start_data, '%Y-%m-%d').date()
        end_date = datetime.strptime(end_data, '%Y-%m-%d').date()
        photos = await search_admin(user_id, tag, rating_low, rating_high, start_date, end_date, db)
        return {"photos": photos}
    except ValueError as e:
        return {"details": BAD_DATE_FORMAT}
